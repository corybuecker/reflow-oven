...
pub async fn create_channel<'channel>(
    ...
    let channel = rmt.channel0.configure_tx(&tx_config).unwrap().with_pin(pin);
...
#[allow(dead_code)]
pub async fn off(channel: &mut Channel<'_, Async, Tx>) -> Result<(), esp_hal::rmt::Error> {
    let pulses = rgb_to_pulses((0, 0, 0)).map_err(|_e| esp_hal::rmt::Error::InvalidDataLength)?;
    channel.transmit(&pulses).await
}

#[allow(dead_code)]
pub async fn blue_led(channel: &mut Channel<'_, Async, Tx>) -> Result<(), esp_hal::rmt::Error> {
    let pulses = rgb_to_pulses((0, 0, 15)).map_err(|_e| esp_hal::rmt::Error::InvalidDataLength)?;
    channel.transmit(&pulses).await
}

#[allow(dead_code)]
pub async fn red_led(channel: &mut Channel<'_, Async, Tx>) -> Result<(), esp_hal::rmt::Error> {
    let pulses = rgb_to_pulses((15, 0, 0)).map_err(|_e| esp_hal::rmt::Error::InvalidDataLength)?;
    channel.transmit(&pulses).await
}

#[allow(dead_code)]
pub async fn green_led(channel: &mut Channel<'_, Async, Tx>) -> Result<(), esp_hal::rmt::Error> {
    let pulses = rgb_to_pulses((0, 15, 0)).map_err(|_e| esp_hal::rmt::Error::InvalidDataLength)?;
    channel.transmit(&pulses).await
}
